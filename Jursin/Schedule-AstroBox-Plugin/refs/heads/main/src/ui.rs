...
fn parse_file(file_name: &str, text: &str) -> Result<Value, String> {
    ...
    } else if parser::is_starlink_json(text) {
        parser::convert_starlink_json_to_json(text)?
    } else {
...
async fn pick_file() {
    ...
    match parse_file(&file_name, &text) {
        Ok(mut parsed) => {
            ...
            let mut slots = parsed.get("timeSlots").and_then(Value::as_array).map_or(0, Vec::len);

            // 星链格式无时间段时，弹第二个文件选时间表
            if slots == 0 && parser::is_starlink_json(&text) {
                log("星链课程表已解析，正在选择时间表...");
                redraw();
                let tt_result = psys_host::dialog::pick_file(
                    &dialog::PickConfig {
                        read: true,
                        copy_to: None,
                    },
                ...
                .await;
                if !tt_result.name.is_empty() {
                    let tt_text = String::from_utf8_lossy(&tt_result.data).into_owned();
                    match parser::convert_timetable_to_time_slots(&tt_text) {
                        Ok(time_slots) => {
                            slots = time_slots.len();
                            parsed["timeSlots"] = Value::Array(time_slots);
                            log(&format!("时间表已合并：{}（{} 个节次）", tt_result.name, slots));
                        }
                        Err(e) => log(&format!("时间表解析失败：{}", e)),
                    }
                } else {
                    log("未选择时间表，课程数据无时间段");
                }
...
fn build_ui(s: &UiState) -> ui_v3::Element {
    ...
    let (file_label, file_color) = if s.file_name.is_empty() {
        ("支持拾光课程表/WakeUp课程表/星链课表/CSES 配置文件", MUTED)
    } else {
