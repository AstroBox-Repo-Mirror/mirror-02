#!/usr/bin/env python3
"""构建 wasm 并整理 dist 产物(基于官方模板 scripts/build_dist.py)。"""
...
def main():
    ...
    if args.package:
        # abp 打包名与 wasm 入口同源（entry 的 stem），不随显示名变化，
        # 避免中文/空格文件名在分发时出错。
        package_name = (
            Path(entry).stem.replace("_", "-")
            if entry
            else make_package_name(plugin_name, crate_name)
        )
