...
/// 启动应用后等待时间（秒）
const LAUNCH_WAIT_SECONDS: u64 = 3;
...
pub async fn resolve_pkg_name(addr: &str) -> Option<String> {
    ...
    update_cache(addr, DEFAULT_PKG_NAME);
    Some(DEFAULT_PKG_NAME.to_string())
}

/// 启动手表端 SmsForwarder 应用并等待就绪
pub async fn launch_and_wait(addr: &str, pkg_name: &str) -> Result<(), String> {
    ...
    let app = apps
        ...
        .ok_or_else(|| format!("手表上未找到 SmsForwarder Client 应用 ({})", pkg_name))?;
