...
impl event::Guest for OtpPlugin {
    fn on_event(event_type: EventType, event_payload: String) -> FutureReader<String> {
        ...
        match event_type {
            ...
            EventType::DeviceAction => {
                ...
                // 设备状态变化时重新注册 interconnect 接收通道（参考官方实现）
...
impl lifecycle::Guest for OtpPlugin {
    fn on_load() {
        logger::init();
        tracing::info!("OTP plugin loaded");

        // 启动周期定时器，每秒刷新UI以更新验证码倒计时
        wit_bindgen::spawn(async move {
            let _ = crate::astrobox::psys_host::timer::set_interval(1000, "tick").await;
        });

        // 启动时注册 interconnect 接收通道（参考 Varclass）
        wit_bindgen::spawn(async move {
